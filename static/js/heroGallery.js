...
const homeGall = 'home-wS6yik-dL0OqH'
...
window.addEventListener('load', () => {
	fetch( `http://sms-api.softsubversion.com/v1/gallery/${homeGall}/` , {
		method: 'GET',
		headers: {
  ...
  .then(data => {
    ...
    for (let i = 0; i < data.images.length; i++) {
      ...
      imageObjectDiv.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2)), url('${data.images[i].image_link}')`;
