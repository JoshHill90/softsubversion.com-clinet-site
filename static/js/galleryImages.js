...
galleryTitle.innerHTML = urlParams.get('name')
...
async function imageSorter(headerImage, dataList) {
    ...
    for (count = 0; count < imageList.length; ++count) {
		...
		colDiv.style.backgroundImage = `url('${img.image_link}')`;
        colDiv.setAttribute('data-mdb-img', img.title);
		...
		slideImg.src = img.image_link;
		slideImg.alt = `Photo Titled ${img.title}`;
...
window.addEventListener('load', () => {
	...
	fetch(`http://sms-api.softsubversion.com/v1/gallery/${urlParams.get('gal')}/`, {
		method: 'GET',
		headers: {
  ...
  .then((data) => {
	...
	const headerImage = data.gallery.header_image;
	const dataList = data.images;
